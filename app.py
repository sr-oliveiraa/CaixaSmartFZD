...
@app.route('/delete_categoria/<int:categoria_id>', methods=['POST'])
...
def gerar_relatorio():
    ...
    # Obtém o filtro de tempo selecionado pelo usuário
    filtro = request.form.get('filtro', 'hoje')
    
    # Determina a data de início com base no filtro
    if filtro == 'hoje':
        data_inicio = date.today()
    elif filtro == 'semana':
        data_inicio = date.today() - timedelta(days=7)
    elif filtro == 'mes':
        data_inicio = date.today().replace(day=1)  # Primeiro dia do mês atual
    else:
        data_inicio = date.today()  # Padrão para hoje se nenhum filtro for selecionado

    # Filtra as transações com base na data de início
    transacoes = Transacao.query.filter(Transacao.data >= data_inicio).all()
...
def search_produto():
    ...
    if not query:
        produtos_list = []  # Retorna uma lista vazia se a consulta estiver vazia
    else:
